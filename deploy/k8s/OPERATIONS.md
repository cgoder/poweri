...
用途：PowerI-Web（monorepo `web/`，ticket 03 已引入；原名 pi-web (agegr)）壳网关模式的文件浏览器与技能菜单（ticket 25）。bridge 侧对应 HTTP 面 `GET /files|/file|/skills`（同 /sessions 模式），改 bridge/gateway 后需重建 poweri-worker + poweri-gateway 镜像并 rollout restart。
