...
| PowerI-Web | monorepo `web/`（ticket 03 引入，v0.8.8 基底 + 适配层定制） | `poweri-web:local`（web/Dockerfile，ticket 10 补齐） | `web/deploy/k8s/poweri-web.yaml`（Deployment+Service 30341，${WEB_USERS}/${GW_USERS} 占位符） |
