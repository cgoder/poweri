...
function sessionIds(body) {
	try {
		return (JSON.parse(body).sessions ?? []).map((s) => s.id);
	} catch {
		return [];
	}
}
async function chatRound(u, p) {
	const nw = await post(
		"/api/agent/new",
		{ type: "prompt", message: "回复一个字：好" },
		{ Authorization: auth(u, p) },
	);
	let sid = "";
	try {
		sid = JSON.parse(nw.body).sessionId ?? "";
	} catch {}
	if (nw.status !== 200 || !/^[0-9a-z]{6,9}-[0-9a-f]{8}$/.test(sid))
		return { status: nw.status, msgs: 0, answer: "" };
	let msgs = 0,
		answer = "";
	for (let i = 0; i < 24; i++) {
		await sleep(5000);
...
try {
	...
	execFileSync("node", ["scripts/gen-k8s.mjs", users.join(","), "--ui"], {
		stdio: "inherit",
	});
	...
	ok(
		"未知用户 401",
		(await get("/", { Authorization: auth("carol", "x") })).status !== 200,
	);
	ok(
		"密码错误 401",
		(await get("/", { Authorization: auth(U1, "wrong") })).status !== 200,
	);
	ok(
		`${U1} 正确凭据 200`,
		(await get("/", { Authorization: auth(U1, P1) })).status === 200,
	);
	ok(
		`${U2} 正确凭据 200`,
		(await get("/", { Authorization: auth(U2, P2) })).status === 200,
	);
	...
	const ids1 = sessionIds(s1.body),
		ids2 = sessionIds(s2.body);
	ok(
		`${U1} 会话列表非空`,
		s1.status === 200 && ids1.length > 0,
		`${ids1.length} 个`,
	);
	ok(
		`${U2} 会话列表非空`,
		s2.status === 200 && ids2.length > 0,
		`${ids2.length} 个`,
	);
	const overlap = ids1.filter((id) => ids2.includes(id));
	ok(
		`两用户会话 id 完全不重叠（token 路由隔离）`,
		overlap.length === 0,
		`交集 ${overlap.length}`,
	);
	...
	ok(
		`${U1} 对话成功且有回答`,
		c1.status === 200 && c1.msgs >= 2 && c1.answer.length > 0,
		`「${c1.answer.slice(0, 10)}…」`,
	);
	const c2 = await chatRound(U2, P2);
	ok(
		`${U2} 对话成功且有回答`,
		c2.status === 200 && c2.msgs >= 2 && c2.answer.length > 0,
		`「${c2.answer.slice(0, 10)}…」`,
	);
