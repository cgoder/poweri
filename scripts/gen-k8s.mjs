...
const users = (process.argv[2] ?? "alice,bob")
	.split(",")
	.map((s) => s.trim())
	.filter(Boolean);
...
if (PIWEB)
	console.warn(
		"⚠ --piweb 已废弃（ticket 27）：上游 @agegr/pi-web 进程内形态旁路网关，产品路径为 PowerI-Web 网关壳；仅保留可回滚",
	);
if (PIWEB2)
	console.warn(
		"⚠ --piweb2 已废弃（ticket 27）：jmfederico 试点（ticket 22）已收口；仅保留可回滚",
	);
...
const GATEWAY_IMAGE =
	process.env.POWERI_GATEWAY_IMAGE ?? "poweri-gateway:local";
...
`
		: "";
const CONFIG_SRC =
	process.env.POWERI_PI_CONFIG_DIR ||
	path.resolve(
...
const cmFiles = ["models.json", "settings.json"].filter((f) =>
	existsSync(path.join(CONFIG_SRC, f)),
);
...
execFileSync(
	...
	[
		...
		"configmap",
		"pi-config",
		...cmFiles.map((f) => `--from-file=${f}=${path.join(CONFIG_SRC, f)}`),
		"-n",
		NS,
	],
	{ stdio: "ignore" },
...
if (!gwApiKey) {
	try {
		const m = JSON.parse(
			readFileSync(path.join(CONFIG_SRC, "models.json"), "utf8"),
		);
...
if (!gwApiKey) {
	console.error(
		"缺少模型 apiKey：POWERI_AI_API_KEY 未设置且 models.json 无字面量密钥",
	);
	process.exit(1);
}
run([
...
const secretLiterals = [
	`POWERI_AI_API_KEY=${gwApiKey}`,
	`POWERI_GATEWAY_USERS=${GW_USERS}`,
];
// ticket 21：pi-web 每实例 Basic Auth 密码（Secret 化，不进 ConfigMap；默认 poweri-<user>，可 POWERI_PIWEB_PASSWORD_<USER> 覆盖）
if (PIWEB)
	for (const u of users)
		secretLiterals.push(
			`PI_WEB_PASSWORD_${u.toUpperCase()}=${process.env[`POWERI_PIWEB_PASSWORD_${u.toUpperCase()}`] ?? `poweri-${u}`}`,
		);
...
const uiToken =
	GW_USERS.split(";")
		...
		.find(([u]) => u === uiUser)?.[1] ?? "";
secretLiterals.push(
	`POWERI_WEB_PASSWORD=${process.env.POWERI_WEB_PASSWORD ?? `poweri-${uiUser}`}`,
);
...
execFileSync(
	...
	[
		...
		"generic",
		"poweri-secrets",
		...secretLiterals.map((l) => `--from-literal=${l}`),
		"-n",
		NS,
	],
	{ stdio: "ignore" },
...
console.log(
	`✓ Secret poweri-secrets（模型 apiKey + 网关用户 token${PIWEB ? " + pi-web 密码" : ""}${UI ? " + PowerI-Web 密码/token" : ""}）`,
);
...
const k8sUsers = users
	...
	.join(";");
applyManifest(path.join(GW_DIR, "deploy", "k8s", "gateway.yaml"), {
	K8S_USERS: k8sUsers,
	GATEWAY_IMAGE,
	WORKER_IMAGE: IMAGE,
	IDLE_MINUTES: process.env.POWERI_WORKER_IDLE_MINUTES ?? "30",
...
if (UI) {
	...
	const webUsers =
		process.env.POWERI_WEB_USERS ??
		users.map((u) => `${u}:poweri-${u}`).join(";");
	applyManifest(path.join(WEB_DIR, "deploy", "k8s", "poweri-web.yaml"), {
		WEB_USERS: webUsers,
		GW_USERS,
		WEB_IMAGE,
	});
	console.log(
		`✓ PowerI-Web UI 已部署（NodePort 30341，账号 ${webUsers}，manifest 来自 poweri-web 仓库）`,
	);
}
	...
	const probe = {
		exec: {
			command: [
				"node",
				"-e",
				"fetch('http://127.0.0.1:30141/',{headers:{Authorization:'Basic '+Buffer.from('pi:'+process.env.PI_WEB_PASSWORD).toString('base64')}}).then(r=>process.exit(r.status<500?0:1)).catch(()=>process.exit(1))",
			],
		},
...
spec:
  ...
  ports:
	...
	console.log(
		`✓ pi-web 已部署：${users.map((u, i) => `piweb-${u} (nodePort ${30241 + i})`).join(", ")}`,
	);
}
...
spec:
  ...
  ports:
	...
	console.log(
		`✓ piweb2 已部署：${users.map((u, i) => `piweb2-${u} (nodePort ${30251 + i})`).join(", ")}`,
	);
}
