...
try {
	...
	const deprecated = execFileSync(
		...
		[
			"-c",
			`timeout 25 node scripts/gen-k8s.mjs ${users.join(",")} --piweb 2>&1 || true`,
		],
		{ encoding: "utf8" },
	);
	ok("--piweb 打印废弃警告", deprecated.includes("已废弃"));
	const deprecated2 = execFileSync(
		"bash",
		[
			"-c",
			`timeout 25 node scripts/gen-k8s.mjs ${users.join(",")} --piweb2 2>&1 || true`,
		],
		{ encoding: "utf8" },
	);
	...
	ok(
		"模型面板单 poweri-gw/agent",
		models.status === 200 && models.body.includes('"poweri-gw"'),
	);
	const sess = await get("/api/sessions", { Authorization: auth });
	let n = 0;
	try {
		n = JSON.parse(sess.body).sessions.length;
	} catch {}
	ok(
		"会话列表经集群内网关非空（UI→gateway svc→worker）",
		sess.status === 200 && n > 0,
		`${n} 个会话`,
	);
	...
	ok(
		"POST /api/agent/new 返回会话 id",
		nw.status === 200 && /^[0-9a-z]{6,9}-[0-9a-f]{8}$/.test(sid),
		sid ? `HTTP ${nw.status} sid=${sid}` : `HTTP ${nw.status}`,
	);
	let msgs = 0,
		answer = "";
	for (let i = 0; i < 24; i++) {
