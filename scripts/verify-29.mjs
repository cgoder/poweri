...
const TOKEN =
	process.env[`POWERI_TOKEN_${USER.toUpperCase()}`] ??
	(USER === "alice" ? "token-a" : "token-b");
	...
	try {
		...
		if (res.ok && res.body) {
			...
			while (true) {
				...
				if (!sid) {
					const m =
						/event: ready\s+data: \{"sessionId":"([0-9a-z]{6,9}-[0-9a-f]{8})"/.exec(
							buf,
						);
					if (m) sid = m[1];
				}
...
try {
	...
	ok(
		"改名后历史端点仍正常（JSONL 完整）",
		(await req("GET", `/v1/sessions/${sid}/messages`)).status === 200,
	);
	ok(
		"改名后 JSONL 导出仍正常",
		(await req("GET", `/v1/sessions/${sid}/jsonl`)).status === 200,
	);
	...
	try {
		const out = execFileSync(
			...
			[
				...
				"sh",
				"-c",
				`ls /home/piuser/.pi/agent/sessions | grep -c ${sid} || true`,
			],
			{ encoding: "utf8" },
		);
		fileGone = out.trim() === "0";
	...
	ok(
		"重复删除 404",
		(await req("DELETE", `/v1/sessions/${sid}`)).status === 404,
	);
	ok(
		"删除不存在 404",
		(await req("DELETE", `/v1/sessions/msbno-such-session-0000`)).status ===
			404,
	);
	ok(
		"无 token 删除 401",
		(await req("DELETE", `/v1/sessions/${sid}`, null, null)).status === 401,
	);
	...
	ok(
		"用户 token 可见自己的计量",
		u1.status === 200 && Array.isArray(u1.body?.records),
		`${u1.body?.records?.length ?? 0} 条`,
	);
	const a1 = await req("GET", "/v1/admin/usage?userId=" + USER, null, ADMIN);
	ok(
		"与 admin 视角一致（同一数据源）",
		a1.status === 200 &&
			(a1.body?.records ?? []).length === (u1.body?.records ?? []).length,
	);
	ok(
		"无 token 计量 401",
		(await req("GET", "/v1/users/me/usage", null, null)).status === 401,
	);
