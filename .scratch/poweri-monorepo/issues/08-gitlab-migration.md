...
- [x] 旧三个 project 只读归档（已交付归档指引与边界说明；gitlab UI 执行属用户协助步骤，见 Comments）
...
- 2026-08-13：GitLab 迁移执行。
  ...
  **用户协助步骤（ticket 定义范围，归档执行）**：`litta-power/poweri-gateway`、`litta-power/poweri-web` 两个旧 project 在 gitlab web UI 设为只读归档（Settings → General → Visibility/Advanced，或 project 归档功能；历史保留）。原 poweri project 即 monorepo 承载者，无需归档。归档完成后本项验收即闭环。
