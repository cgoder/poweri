...
- **范围：所有用户行为统一**（用户 2026-08-03 决定"行为一致是基本原则，优化是额外附加"）。
  预置用户（alice/bob）与按需用户同一套规则：空闲超时 → 网关缩容到 0；请求 → 网关拉起。
- **HPA 已移除**（`deploy/k8s/hpa.yaml` 删除）：CPU-only HPA 不支持 `minReplicas: 0`
  （API server 拒绝，需 Object/External 指标才允许缩到 0），与"统一缩到 0"互斥。
  负载扩容（1→3）属优化，需要指标源后另行决策；metrics-server 不再需要。
- **缩容方式：scale-to-0，不删 PVC/Service**。数据（会话/workspace 在 PVC）随用户保留，
  下次请求自动拉起（冷启动 30-90s）。删 PVC = 数据丢失，不做。
...
- **孤儿回收**：网关重启后（lastSeen 为空、用户不在内存路由表），经 `labelSelector=role=worker`
  枚举孤儿 worker 一并缩容（RBAC deployments 增加 list）。
...
- `pods.mjs`：`lastSeen`/`touch`/`scaleWorker`（merge-patch PATCH replicas）/`sweepIdleWorkers`
  （60s 巡检：所有用户空闲统一缩容 + 孤儿回收），`ensureK8sWorker` 增加"已缩容 → PATCH 1 拉起"分支，
...
`node scripts/verify-32.mjs alice,carol`（IDLE_MINUTES=1）— **10/10 通过**（统一版）：
alice（预置）与 carol（按需开通）各走一轮：首次对话 → 空闲 1min 统一缩容到 0 +
PVC 保留 → 再次对话自动拉起（replicas 0→1）→ 缩容前会话仍在 PVC（数据未丢）。
`node scripts/verify-30.mjs` 同步适配（无 HPA 断言）。
...
- 2026-08-03：实现 + 验证 10/10（先动态版）。缩容调试中发现两个连带 bug 一并修掉：
  拉起路径失效（缩容后仍在路由表，永远连 0 副本 worker）与 `/sessions/<id>/messages`
  无 try/catch 崩进程。
- 2026-08-03（用户决定统一）：所有用户行为一致为基本原则 → 静态用户也参与缩容；
  HPA 因 CPU-only 不支持 min=0 而移除（负载扩容属优化，需指标源后另行决策）。
